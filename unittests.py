...
class TestArgmax(unittest.TestCase):
    def test_argmax_largest_twice_at_end(self):
        t = AssemblyTest(self, "argmax.s")
        # create an array in the data section
        array0 = t.array([-25, -1, 5, 1, 2, -7, -1234, 5])
    ...
    def test_argmax_largest_at_end(self):
        t = AssemblyTest(self, "argmax.s")
        # create an array in the data section
        array0 = t.array([5, 1, 2, -7, 8])
        # load address of the array into register a0
        t.input_array("a0", array0)
        # set a1 to the length of the array
        t.input_scalar("a1", len(array0))
    ...
    def test_argmax_largest_twice(self):
        t = AssemblyTest(self, "argmax.s")
        # create an array in the data section
        array0 = t.array([5, 1, 5, -7, 0])
        # load address of the array into register a0
        t.input_array("a0", array0)
        # set a1 to the length of the array
        t.input_scalar("a1", len(array0))
...
class TestClassify(unittest.TestCase):
    ...
    # Add this method to the TestClassify class
    def test_classify_mnist(self):
        t = self.make_test()
        outfile = "../MNIST/inputs/output.bin"
        args = [
            "../MNIST/m0.bin",    # Adjust path as needed
            "../MNIST/m1.bin",    # Adjust path as needed
            "../MNIST/inputs/mnist_input0.bin",    # Adjust path as needed
            outfile,
        ]
        # Set to 0 for printing classification, 1 for silent
        t.input_scalar("a2", 0)  
        t.call("classify")
        # No need to check specific classification since we don't know it yet
        t.execute(args=args)
        print(f"Output matrix saved to {outfile}")
